...
class OfflineFirstAstronomyPictureRepositoryTest {
    ...
    @Test
    fun repository_astronomy_picture_detail_returns_astronomy_picture_from_dao_if_exists() =
        runTest {
            // Given FakeNetworkWithResult
            val astronomyPictureDao = FakeAstronomyPictureDao()
            val networkDataSource = FakeNetworkWithResult()
            ...
            // When
            repository.loadPictures()
            val testDates = fakeNetworkAstronomyResources.mapNotNull { it.date }.plus(
                // Add date that doesn't exist in DAO, should return null from repository
                LocalDate.parse("1981-01-01")
            )

            // Then
            testDates.forEach { localDate ->
                assertEquals(
                    astronomyPictureDao.getAstronomyPicture(localDate.toString()).first()
                        ?.asExternalModel(),
                    repository.getPictureDetail(localDate).first()
                )
            }
        }
}
...
private class FakeAstronomyPictureDao : AstronomyPictureDao {
    ...
    override fun getAstronomyPicture(date: String): Flow<RoomAstronomyPicture?> = flow {
        emit(data.find { it.date.toString() == date })
    }
...
private class FakeNetworkWithResult : PlanetaryNetworkDataSource {
    override suspend fun getPictures(): Result<List<NetworkAstronomyResource>> =
        Result.success(fakeNetworkAstronomyResources)
}
